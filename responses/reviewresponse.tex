...
We agree with the reviewer that paper is not comprehensive in studying the effect of the proposed mechanism for various traffic scenarios in the Internet. We considered that for our future research work and the corresponding text is updated.
...
2. RTO is used mainly to suggest that it is the standard (not Linux) TCP retransmission policy. RTO is used to elevate the importance of RTO timer in the retransmission policy.\\
3. In the case of Figure 2 b), PTO is very close to RTO and the second packet is recovered with ER when TLP is used unlike resending all unacked packets on RTO. \\
4. Due to the page limit constraints, it is not feasible to provide the figures for asymmetric delays. However the current figures could be updated to enhance the visualization. \\
   Cli-I1 is Client Interface 1. It could be changed to Inf1-Client. We have simplified the timing figures to make it clean. \\
5. All transmissions are limited by congestion window and that covers the increased load scenario. We consider the suggestion for our future research.\\
